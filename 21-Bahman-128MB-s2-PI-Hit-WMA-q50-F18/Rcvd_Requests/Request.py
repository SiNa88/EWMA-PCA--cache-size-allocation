...
def MyStem():
	...
	plt.plot(x_axis,Smooth20,'o-g',label="Rtr8")
	...
	plt.title('SnapShot of Pending Interest Tables(PIT)')
	plt.show()
	#plt.scatter(x_axis,Smooth_mean)
	print np.around(np.average(np.copy(Smooth_Homo)),6),"\n", np.around(np.average(np.copy(Smooth_Deg)),6),"\n", np.around(np.average(np.copy(Smooth_mean)),6),"\n"
	print Smooth_Homo,"\n"
	print Smooth_Deg,"\n"
	print Smooth_mean,"\n"

	plt.plot(x_axis,Smooth_Homo,'.-c',label="Homogeneous")
	plt.plot(x_axis,Smooth_Deg,'o-b',label="Degree-Centrality")
	plt.plot(x_axis,Smooth_mean,'^-r',label="ProposedMethod")
	plt.xlim([-0.1,tedaad+0.1])
	plt.ylim([10,50])
	plt.xlabel('Time(*100 Seconds)')
	plt.ylabel('Pending Requests')
